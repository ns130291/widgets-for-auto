...
public class PickAppWidgetActivity extends Activity {
    ...
    private static AsyncRun runAsync(@NonNull AsyncRun.Run background, @Nullable AsyncRun.Run after) {
        return (AsyncRun) new AsyncRun(background, after).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Log.i("widgetslist", "onCreate pick widget");
        ...
        public void setContent(MenuItem content) {
            ...
            if (content instanceof ItemWidget) {
                ...
                task = runAsync(t -> {
                    ...
                    int minHeight = Math.round(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 48, textView.getResources().getDisplayMetrics()));
                    if (h < minHeight) {
                        h = minHeight;
